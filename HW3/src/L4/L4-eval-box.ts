...
import { Result, bind, mapv, mapResult, makeFailure, isOk, makeOk } from "../shared/result";
...
const applicativeEval = (exp: CExp, env: Env): Result<Value> => {
    ...
    isTraceExp(exp) ? evalTraceExp(env, exp) : // yoesf
    exp;
...
// HW3
const evalTraceExp = (env: Env ,exp: TraceExp ): Result<void> =>
    // yoesf
    ...
    makeFailure("evalTraceExp failed"))
...
const applyTracedClosure = (proc: TracedClosure, args: Value[]): Result<Value> => {
    ...
    cnt++;
    let value = applyClosure(proc.closure, args)
    cnt--;
    printPostTrace(isOk(value)? value.value: "  ", cnt)
    return value
}
