...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LetPlusExp | LitExp; //radwan
...
export interface LetPlusExp {tag: "LetPlusExp"; bindings: Binding[]; body: CExp[];}; //radwan
...
export const makeLetPlusExp = (bindings: Binding[], body: CExp[]): LetPlusExp =>
    ({tag: "LetPlusExp", bindings: bindings, body: body}); // radwan
...
export const isLetPlusExp = (x: any): x is LetPlusExp => x.tag === "LetPlusExp"; // radwan
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isLetPlusExp(x); // radwan
...
export const parseL31SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>
    ...
    op === "let*" ? parseLetPlusExp(first(params), rest(params)) :   // radwan
    makeFailure("Never");
